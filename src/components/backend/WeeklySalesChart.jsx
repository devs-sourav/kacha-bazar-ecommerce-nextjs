...
export default function WeeklySalesChart() {
  ...
  return (
    <div className='dark:bg-slate-700 bg-slate-50 p-8 rounded-lg shadow-lg'>
        <h2 className='text-xl dark:text-slate-50 text-slate-800 font-bold mb-3'>Weekly Charts</h2>
        <div className="">
          {/* Tabs Here*/}
          <div className="text-sm font-medium text-center text-slate-500 border-b border-slate-200 dark:text-gray-400 dark:border-gray-400">
                  ...
                  tabs.map((tab,i)=>(
                    <li className="me-2" key={i}>
                      <button onClick={()=>setChartToDisplay(tab.type)} className={chartToDisplay == tab.type ? (chartToDisplay == 'sales' ?
                        ...
                        :
                        "inline-block p-4 border-b-2 border-transparent rounded-t-lg hover:text-slate-800 hover:border-gray-300 dark:hover:text-gray-100"}>{tab.title}</button>
                    </li>
                  ))
                }
              </ul>
